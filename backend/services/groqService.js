try {
    ...
    async function callGroqWithRetry(prompt, maxTokens = 400, temp = 0.2, retries = 2) {
...
Output JSON Format Strict Requirement:
{
  ...
  "confidence": 0.9
        ...
        const text = await callGroqWithRetry(prompt, 500, 0.2);
...
User complaints today: ${userStats?.todayCount || 0}
Similar complaints nearby (24h): ${userStats?.similarCount || 0}
User warnings: ${userStats?.warnings || 0}
...
Return ONLY JSON:
{
  ...
  "confidence": 0.92
        ...
        const text = await callGroqWithRetry(prompt, 300, 0.1);
        return safeParseJSON(text);
    }
    ...
    async function generateLetter(complaint, userName) {
        ...
        try {
            ...
            dept = DEPARTMENT_MAP[complaint.category]?.name || dept;
        } catch (e) { }
...
Under 350 words. Return only the letter text.`;
        return callGroqWithRetry(prompt, 600, 0.3);
    }
...
Be specific and reassuring. Simple English. Return only the message.`;
        return callGroqWithRetry(prompt, 120, 0.6);
    }
...
Under 180 words. Natural spoken language.`;
        return callGroqWithRetry(prompt, 300, 0.4);
    }
...
User: ${userName}`;
    ...
    console.error('Groq service failed to initialize:', globalError.message);

    // Safe fallback implementations
    const fallback = async () => ({
        title: 'Fallback Title', description: 'Fallback description', category: 'Other', priority: 'Low', isFake: false, recommendation: 'approve'
    });
    const fallbackText = async () => "AI Generation currently unavailable.";

    module.exports = {
        analyzeComplaint: fallback,
        detectFake: fallback,
        generateLetter: fallbackText,
        statusMessage: fallbackText,
        callScript: fallbackText,
        chatbotReply: fallbackText
    };
}
