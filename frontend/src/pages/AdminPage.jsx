...
export default function AdminPage() {
  ...
  const [govTickets, setGovTickets] = useState([]);
  const [checkingAll, setCheckingAll] = useState(false);

  const fetchGovTickets = async () => {
    try {
      const { data } = await govAPI.getAllAdminGovTickets();
      setGovTickets(data.tickets);
    } catch { toast.error('Failed to load gov tickets'); }
  };

  useEffect(() => {
    if (activeTab === 'Gov Tickets') fetchGovTickets();
  }, [activeTab]);
  ...
  const statCards = stats ? [
    { label: 'Total', val: stats.total, icon: '📋', color: 'text-gray-800 border-gray-300', bg: 'bg-gray-100' },
    { label: 'Reported', val: stats.reported, icon: '🔴', color: 'text-saffron-dark border-saffron', bg: 'bg-saffron-pale' },
    { label: 'In Progress', val: stats.inProgress, icon: '⚙️', color: 'text-gray-600 border-gray-400', bg: 'bg-gray-200' },
  ...
  return (
    <div className="pt-20 min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {/* Tabs */}
        <div className="flex gap-4 border-b border-gray-200 mb-6 font-heading text-sm font-bold uppercase tracking-wider overflow-x-auto">
          {['Overview', 'Complaints', 'Automation', 'Gov Tickets'].map(t => (
            <button key={t} onClick={() => setActiveTab(t)} className={`pb-3 px-2 transition-colors border-b-2 whitespace-nowrap ${activeTab === t ? 'border-saffron text-saffron' : 'border-transparent text-gray-500 hover:text-gray-800'}`}>
              {t === 'Automation' ? '🤖 Automation' : t === 'Gov Tickets' ? '🏛️ Gov Tickets' : t}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'Gov Tickets' && (
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center">
              <h3 className="font-heading font-bold text-lg">Government Portal Tracker</h3>
              <button
                onClick={async () => {
                  setCheckingAll(true);
                  const tid = toast.loading('Iterating all open tickets...');
                  try {
                    for (const t of govTickets) {
                      if (t.currentStatus !== 'Disposed — Action Taken') {
                        await govAPI.checkStatus(t.ticketId);
                      }
                    }
                    toast.success('All open tickets synced!', { id: tid });
                    fetchGovTickets();
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100 text-xs font-bold text-gray-500 uppercase tracking-wider text-left">
                ...
                <tbody>
                  {govTickets.map(t => (
                    <tr key={t._id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      <td className="px-4 py-3 max-w-[150px] truncate text-xs font-semibold">{t.complaint?.title}</td>
                      <td className="px-4 py-3 text-xs text-gray-600">{t.user?.name}</td>
                      <td className="px-4 py-3"><span className="text-[10px] font-bold bg-gray-100 rounded px-2 py-0.5">{t.portalName}</span></td>
                      <td className="px-4 py-3 font-mono text-xs font-bold text-gray-700">{t.ticketId}</td>
                      <td className="px-4 py-3"><GovStatusBadge status={t.currentStatus} /></td>
                      <td className="px-4 py-3 text-xs text-gray-400">{timeAgo(t.lastChecked)}</td>
                      <td className="px-4 py-3">
                        <button onClick={async () => {
                          const tid = toast.loading('Checking...');
                          try { await govAPI.checkStatus(t.ticketId); toast.success('Updated', { id: tid }); fetchGovTickets(); }
                          catch { toast.error('Failed', { id: tid }); }
                        }} className="bg-gray-100 text-[10px] uppercase font-bold text-gray-600 py-1.5 px-3 rounded-lg hover:bg-saffron hover:text-white transition-colors">Check</button>
                      </td>
                    </tr>
                  ))}
                  {govTickets.length === 0 && <tr><td colSpan="7" className="text-center py-8 text-gray-400">No tickets found</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
