            ...
            <div style={modalStyle}>
                ...
                <div style={{ background: 'linear-gradient(135deg,#FF9933,#E8720C)', padding: '1.5rem', color: 'white' }}>
                    <div style={{ fontSize: '1.4rem', fontWeight: '800', fontFamily: 'Rajdhani,sans-serif' }}>
                        {step === 'permission' && '📞 AI Officer Call'}
                        {step === 'script' && '🤖 Call Script Preview'}
                        {step === 'calling' && '📡 Call In Progress...'}
                        {step === 'done' && '✅ Call Initiated'}
                    </div>
                ...
                <div style={{ padding: '1.5rem' }}>
                    ...
                    {step === 'permission' && (
                        <>
                            <div style={{ background: '#FFF3E0', borderRadius: '16px', padding: '1.25rem', marginBottom: '1.25rem', border: '1px solid rgba(255,153,51,.25)' }}>
                                <div style={{ fontSize: '1rem', fontWeight: '800', marginBottom: '.5rem', color: '#E8720C' }}>🤖 JantaVoice AI will:</div>
                                {['Call the responsible department officer', 'Explain your complaint professionally', 'Ask for resolution timeline', 'Record the full conversation', 'Generate a transcript for your records'].map(item => (
                                    <div key={item} style={{ display: 'flex', alignItems: 'center', gap: '.5rem', fontSize: '.85rem', color: '#555', marginBottom: '.3rem' }}>
                                        <span style={{ color: '#138808', fontWeight: '800' }}>✓</span>{item}
                                    </div>
                                ))}
                            </div>

                            <div style={{ background: '#F0FDF4', borderRadius: '12px', padding: '1rem', marginBottom: '1.25rem', border: '1px solid rgba(19,136,8,.2)', fontSize: '.82rem', color: '#555' }}>
                                🔒 <strong>Privacy:</strong> The call will be made from JantaVoice's number. Your personal phone number will NOT be shared with the department.
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: '.75rem' }}>
                                ...
                                <button onClick={handlePermissionGrant} disabled={loading} style={{ flex: 2, padding: '11px', background: 'linear-gradient(135deg,#FF9933,#E8720C)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '800', fontFamily: 'Nunito,sans-serif', fontSize: '1rem' }}>
                                    {loading ? '⚙️ Generating Script...' : '✅ Yes, Call Department'}
                                </button>
                            </div>
                        </>
                    )}
                    ...
                    {step === 'script' && (
                        <>
                            ...
                            <div style={{ display: 'flex', gap: '.75rem' }}>
                                ...
                                <button onClick={handleConfirmCall} disabled={loading} style={{ flex: 2, padding: '11px', background: 'linear-gradient(135deg,#138808,#0A5C04)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '800', fontFamily: 'Nunito,sans-serif', fontSize: '1rem' }}>
                    ...
                    {step === 'calling' && (
                        <div style={{ textAlign: 'center', padding: '2rem 1rem' }}>
                            <div style={{ fontSize: '4rem', marginBottom: '1rem', animation: 'floatY 1.5s ease infinite' }}>📡</div>
                    ...
                    {step === 'done' && (
                        <div style={{ textAlign: 'center', padding: '1.5rem 1rem' }}>
                            ...
                            <button onClick={onClose} style={{ padding: '11px 32px', background: 'linear-gradient(135deg,#FF9933,#E8720C)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '800', fontFamily: 'Nunito,sans-serif' }}>
                                Got it 👍
                            </button>
                        </div>
                    )}
